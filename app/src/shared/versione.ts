/** Analisi e confronto SemVer 2.0, condivisi da renderer e processo main. */

const SEMVER =
  /^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(?:-((?:0|[1-9]\d*|\d*[A-Za-z-][0-9A-Za-z-]*)(?:\.(?:0|[1-9]\d*|\d*[A-Za-z-][0-9A-Za-z-]*))*))?(?:\+[0-9A-Za-z-]+(?:\.[0-9A-Za-z-]+)*)?$/

interface VersioneAnalizzata {
  maggiore: number
  minore: number
  patch: number
  prerelease: string[]
...
export function analizzaVersione(valore: unknown): VersioneAnalizzata | null {
...
export function versioneValida(valore: unknown): valore is string {
...
export function confrontaVersioni(a: string, b: string): number {
  ...
  for (const campo of ['maggiore', 'minore', 'patch'] as const) {
